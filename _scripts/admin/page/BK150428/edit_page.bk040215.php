		...
		if($data_page and $data_section)
		{
			...
			if(isset($_POST['action']) and $_POST['action']=='edit_page')
			{
				...
				foreach ($_POST as $key => $value) 
				{
					...
					$section = strlen(trim($_POST[$key]['section'])) ? true : false;
				...
				if($count_error_image == 0 and $count_error_fields_required == 0)
				{
					...
					foreach ($_POST as $key => $value)
					{
						$chk_private = trim($_POST[$key]['chk_private']) == 'on' ? 1 : 0;
						$content = FX_System::saveStrDb(trim($_POST[$key]['content']));
				...
				else
				{
					foreach ($_POST as $key => $value)
					{
						$data_page_fill[$key] = array('fx_page_id' => $_POST[$key]['page_id'],
