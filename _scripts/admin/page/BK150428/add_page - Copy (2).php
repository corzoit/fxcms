...
{   
	...
	{							
		...
		$options = array("upload_dir" => $ruta, "upload_url" => $ruta, "entities" => "FX_Page", "fx_page_id" => $_REQUEST['fx_page_id']);
	}	

	$upload_handler = new FX_UploadHandler($options);	
	$file = $upload_handler->handle_file_upload($uploaded_file, $name, $size, $type, $error,
            $index = null, $content_range = null);
	

	$file = $upload_handler::handle_file_upload(
   ...
   /* return handle_file_upload($uploaded_file, $name, $size, $type, $error,
            $index = null, $content_range = null);
	*/
...
function show_galery($fx_page_id)
{
	?>
			...
			<div class="col-sm-3">
				<div class="panel panel-green">
			    	...
			    	<div class="panel-body">				    	
			        	<img class="img" src="http://www.horamundial.com/images/icon-5.png">
			    	</div>
				</div>		
			</div>			
		</div>
		<script type="text/javascript">	
		var route_page_id = '<?=$fx_page_id?>'
		var url = '<?=FX_System::url("admin/page/add/?show_galery=true")?>'; 	
		$('#fileupload').fileupload({  
			url :   url,    
		    dataType: 'json',
	        formData: [
	        	{ name: 'ruta', value: 'file/img/media/pageid_'+route_page_id+'/' },
	        	{ name: 'entities', value: 'FX_Page' },
	        	{ name: 'fx_page_id', value: route_page_id },
	        ],// route
