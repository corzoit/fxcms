	...
	{			
		...
		{					
			if(is_array($value_post))
			{
				...
				if(LANG_SYS == $key_post and ($section == false or $title_page == false))
			    {
			    ...
			    else if(LANG_SYS != $key_post and $title_page == false)
			    {
			    	$error = $_LANG[LANG_SYS]['add_pag_msg_error_required'];
			    	$count_error_fields_required = $count_error_fields_required + 1;
			    	$errors[$key_post]['error_field_required'] = $error;
		...
		if($count_error_image == 0 and $count_error_fields_required == 0)
		{
			...
			{					
				FX_System::redirect(FX_System::url("admin/media/?fx_page_id=".$new_id),true);
			}
