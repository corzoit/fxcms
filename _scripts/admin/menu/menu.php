			...
			<form role="form" method="POST">		
			    <div class="form-group">
				    ...
				    <select class="form-control" name = "pos_menu">
				        ...
				        <option value = "bottom"><?=$_LANG[LANG_SYS]['menu_mng_add_menu_sel_pos_menu_bottom']?></option>
				    </select>
				</div>
				...
				<button class="btn btn-danger" type="button" onclick="closeFancy()" ><?=$_LANG[LANG_SYS]['menu_mng_add_menu_btn_close']?></button>				
			</form>
		</div>	
	<?php    
	    exit();
	...
	{					
		...
		else
		{
			...
			foreach ($all_language as $key => $language) 
			{
				...
				$owner_id = '';
				$creation_dt = date("Y-m-d H:i:s");
				$title = FX_System::saveStrDb($_POST['title'][$language]);
